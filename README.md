...
| Lua 5.1 table lengths | Heals stale 5.0 table lengths: when a table's keys were cleared without `table.setn` (what Lua-5.1-detecting libraries like the Ace2 era's Dewdrop/FuBar skip on purpose), `table.getn` / `insert` / `remove` / `concat` / `sort` return the true border, the Lua 5.1 answer, instead of a stale count. Fixes the silent breakage of 5.1-probing libraries (e.g. minimap-button menus). Vararg `arg` tables (explicit `n` field) and `table.setn` users are unaffected. See [Stale table lengths](docs/API.md#stale-table-lengths-51-healing). |
